/**1. Where Have My Four Letter Words Gone? Create a function that takes an array of strings. Return all words in the array that are exactly four letters.

Examples:

isFourLetters(["John", "James", "Jack", "Jeanne"]) ➞ ["John", "Jack"]
isFourLetters(["Tomato", "Corn", "Lettuce"]) ➞ ["Corn"]
isFourLetters(["Dog", "Cat", "Deer"]) ➞ ["Deer"] */

let array = ["John", "James", "Jack", "Jeanne"];

let isFourLetters = array.filter((item) => item.length === 4);

//console.log("fourLetters==>", isFourLetters); // [ 'John', 'Jack' ]

/**2. Months. Create a function that takes a number (from 1 to 12) and return its corresponding month name as a string.

Examples:

monthName(3) ➞ "March"
monthName(12) ➞ "December"
monthName(6) ➞ "June" */

function getMonth(number) {
  const month = [
    "January",
    "February",
    "March",
    "April",
    "may",
    "June",
    "July",
    "August",
    "September",
    "October",
    "November",
    "December",
  ];

  return month[number - 1];
}

//console.log(getMonth(8)); // august

/**3. Amplify the Multiples of 4. Create a function that takes an integer and returns an array of integers ascending from 1 to the given number, where:

For each number in the array that can be evenly divided by 4, that number is amplified by 10 (i.e. return 10 times the number).
If the number cannot be divided evenly by 4, simply return the number.
The given integer will always be equal to or greater than 1.
Include the given number (the number in the parameters).
Examples:

amplify(4) ➞ [1, 2, 3, 40]
amplify(3) ➞ [1, 2, 3]
amplify(25) ➞ [1, 2, 3, 40, 5, 6, 7, 80, 9, 10, 11, 120, 13, 14, 15, 160, 17, 18, 19, 200, 21, 22, 23, 240, 25] */

let box = [];
let multi = 0;

function amplify(number) {
  for (i = 1; i <= number; i++) {
    if ([i] % 4 === 0) {
      multi = [i] * 10;
      box.push(multi);
    } else {
      box.push([i]);
    }
  }
  return box;
}

//console.log(amplify(25));

/**4. One is not like the others... Create a function that takes an array of numbers and return the number that's unique.

Examples:

unique([3, 3, 3, 7, 3, 3]) ➞ 7
unique([0, 0, 0.77, 0, 0]) ➞ 0.77
unique([0, 1, 1, 1, 1, 1, 1, 1]) ➞ 0 */

function getUnique(array) {
  // array.sort((a, b) => a - b);
  for (let i = 0; i < array.length; i++) {
    if (array[i] !== array[i + 1]) {
      return array[i + 1];
    }
  }
}

console.log("getUnique1", getUnique([3, 3, 3, 7, 3, 3])); //

//------------------------------------------------------------

function getUniqueNumber(arr) {
  let counter = {};

  for (item of arr) {
    counter[item] = counter[item] ? counter[item] + 1 : 1;
  }

  for (let key in counter) {
    console.log(key);
    if (counter[key] == 1) {
      return key;
    }
  }
  console.log(counter);
  return;
}

console.log("uniqueNumber==>", getUniqueNumber([0, 0, 0.77, 0, 0])); //

//------------------------------------------------------------------------------------------

/**5. Word Ranking. Create a function that takes a string of words and returns the highest scoring word. Each letter of a word scores points according to it's position in the alphabet: a = 1, b = 2, c = 3, etc.

The returned string should only contain alphabetic characters (a-z).
Preserve case in the returned string (see 4th example above).
Examples:

wordRank("The quick brown fox.") ➞ "brown"
wordRank("Nancy is very pretty.") ➞ "pretty"
wordRank("Check back tomorrow, man!") ➞ "tomorrow"
wordRank("Today is Wednesday.") ➞ "Wednesday" */

function WordRank(sentence) {
  let abc = "abcdefghijklmnopqrstuvwxyz";

  function getScoreOfWord(word) {
    let sum = 0;
    let strArr = word.split("");
    strArr.forEach((chr) => (sum += abc.indexOf(chr) + 1));
    return sum; //
  }

  let wordArray = sentence.split(" ");
  let highestWord = wordArray.reduce((acc, word) => {
    if (getScoreOfWord(word) > getScoreOfWord(acc)) {
      acc = word;
    }
    return acc;
  }, "");

  return highestWord;
}

console.log(WordRank("The quick brown fox."));
console.log(WordRank("Nancy is very pretty."));
console.log(WordRank("Check back tomorrow, man!"));

//------------------------------------------------------------------

/**6. c4n y0u r34d th15? Create a function that takes a string as an argument and returns a coded (h4ck3r 5p34k) version of the string. NB: for your program to work properly, the function should replace all 'a's with 4, 'e's with 3, 'i's with 1, 'o's with 0, and 's's with 5.

Examples:

hackerSpeak("javascript is cool") ➞ "j4v45cr1pt 15 c00l"
hackerSpeak("programming is fun") ➞ "pr0gr4mm1ng 15 fun"
hackerSpeak("become a coder") ➞ "b3c0m3 4 c0d3r" */

let getHacking = (string) => {
  const vowels = {
    a: 4,
    e: 3,
    i: 1,
    o: 0,
    s: 5,
  };

  let stringToArr = string.split("").map((el) => {
    for (let key in vowels) {
      if (el === key) {
        el = vowels[key];
      } else {
        el = el;
      }
    }
    return el;
  });
  return stringToArr.join("");
};

let hackerSpeak = getHacking;

console.log("HackerSpeak=>", hackerSpeak("javascript is cool")); //j4v45cr1pt 15 c00l
console.log("HackerSpeak2 ==>", hackerSpeak("programming is fun")); // pr0gr4mm1ng 15 fun

//--------------------------------------------------------------------------------------------

/* 1. Is it Symmetrical? Create a function that takes a number as an argument and returns true or false depending on whether the number is symmetrical or not. NB: A number is symmetrical when it is the same as its reverse.

Examples:
isSymmetrical(7227) ➞ true
isSymmetrical(12567) ➞ false
isSymmetrical(44444444) ➞ true
isSymmetrical(9939) ➞ false
isSymmetrical(1112111) ➞ true*/

let isSymmetrical = (number) => {
  justNumber = number.toString().split("");
  reverseNumber = number.toString().split("").reverse();
  if (justNumber === reverseNumber) {
    return true;
  } else {
    return false;
  }
};

console.log(isSymmetrical(7853));

/* 2. snake_case ➞ camelCase 
Create a function toCamelCase() that takes a single string in snake_case and converts it into camelCase.

Examples:
toCamelCase("hello_world") ➞ "helloWorld"
toCamelCase("javascript_is_fun") ➞ "javaScriptIsFun"*/

let toCamelCase = (string) => {
  let stringArr = [];
  string.split("_").map((item, index) => {
    if (index !== 0) {
      word = item
        .split("")
        .map((letter, i) => (i === 0 ? letter.toUpperCase() : letter))
        .join("");
      stringArr.push(word);
      return stringArr;
    } else {
      stringArr.push(item);
    }
  });
  return stringArr.join("");
};

console.log("camelcase=>", toCamelCase("hello_world"));

// -----------------------------------------------------------------------

/* 3. Pig Latin Translation. Create a function that takes a string of words and moves the first letter of each word to the end of it, then adds 'ay' to the end of the word. This is a basic form of "Pig Latin".

Move the first letter of each word to the end of the word.
Add "ay" to the end of the word.
Words starting with a vowel (A, E, I, O, U) append "way" to the end instead.

Extra Practice
Preserve proper capitalization as in the examples below.
Examples:
pigLatin("Cats are great pets.") ➞ "Atscay areway reatgay etspay."
pigLatin("Tom got a small piece of pie.") ➞ "Omtay otgay away allsmay iecepay ofway iepay."
pigLatin("He told us a very exciting tale.") ➞ "Ehay oldtay usway away eryvay excitingway aletay."*/

function pigLatin(sentence) {
  let vowels = "aeiou";
  let strArray = sentence.toLowerCase().split(" ");

  let result = [];
  strArray.map((el) => {
    if (vowels.includes(el[0])) {
      result.push(el + "way");
    } else {
      result.push(el.slice(1) + el.substring(0, 1) + "ay");
    }
  });
  let resultString = result.join(" ");
  resultString =
    resultString[0].toUpperCase() +
    resultString.substring(1).replace(".", "") +
    ".";
  return resultString;
}

console.log(pigLatin("Cats are great pets."));
console.log(pigLatin("Tom got a small piece of pie."));
